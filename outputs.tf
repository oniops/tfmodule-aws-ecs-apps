...
output "ecr_name" {
  value = local.create_ecr_repository ? try(aws_ecr_repository.this[0].name, "") : local.ecr_name
}
...
output "target_group_arn" {
  value = local.enable_default_tg ?  try(aws_lb_target_group.this[0].arn, "") : try(aws_lb_target_group.blue[0].arn, "")
}
